...
fn main() -> anyhow::Result<()> {
    let opts: options::ExoOptions = options::ExoOptions::parse();
    ...
    let result = match &opts.subcommand {
        ...
        SubCommand::Config(config_opts) => match &config_opts.command {
            ConfigCommand::Validate => config::validate(&opts, config_opts),
            ConfigCommand::Standalone(standalone_opts) => {
                config::standalone(&opts, config_opts, standalone_opts)
            }
        },
    };
