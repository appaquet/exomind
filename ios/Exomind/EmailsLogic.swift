...
class EmailsLogic {
    static func sanitizeHtml(_ html: String) -> String {
        let builderFunc = JSBridge.instance.jsContext.evaluateScript("exomind.emailsLogic.sanitizeHtml")
        return builderFunc!.call(withArguments: [html]).toString()
    }

    static func plainTextToHtml(_ text: String) -> String {
        let builderFunc = JSBridge.instance.jsContext.evaluateScript("exomind.emailsLogic.plainTextToHtml")
        return builderFunc!.call(withArguments: [text]).toString()
    }

    static func splitOriginalThreadHtml(_ html: String) -> (String, String) {
        let builderFunc = JSBridge.instance.jsContext.evaluateScript("exomind.emailsLogic.splitOriginalThreadHtml")
