    ...
    PS: pending::PendingStore,
{
    ...
    pub fn run_garbage_collector(&self) -> Result<Vec<EntityMutation>, Error> {
        ...
        // make sure that the deletion mutations generated by last GC have been applied
        // to the chain index so that we don't re-collect them again.
        let min_depth = self
            .config
        ...
        if last_run_block_height > 0 && last_chain_index_height - last_run_block_height < min_depth
        {
            return Ok(Vec::new());
        }
