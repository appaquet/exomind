    ...
    PS: pending::PendingStore,
{
    ...
    gc: GarbageCollector,
    gc_last_run_block_height: AtomicU64,
}
    ...
    PS: pending::PendingStore,
{
    ...
    pub fn open_or_create(
        ...
        let mut index = EntityIndex {
            ...
            gc_last_run_block_height: AtomicU64::new(0),
        };
    ...
    pub fn run_garbage_collector(&self) -> Result<Vec<EntityMutation>, Error> {
        ...
        let min_depth = self
            .config
            .chain_index_min_depth
            .max(self.config.chain_index_depth_leeway);
        if last_chain_index_height - last_run_block_height < min_depth {
            // deletion mutations that may have been generated by last GC may not have been
            // applied to the chain index yet. we need to run for them to be applied before
            // running again so that we don't re-collect them again.
            return Ok(Vec::new());
        }
