...
impl SynchronizedThread {
    ...
    pub fn from_gmail(
        ...
        let thread_create_date = emails.first().and_then(|email| email.proto.received_date);
        let thread_modification_date = emails.last().and_then(|email| email.proto.received_date);
